...
//   std::cout << "txn1:" << txn1->GetReadTs() << " " << txn1->GetCommitTs() << std::endl;

  TxnMgrDbg("after commit", bustub->txn_manager_.get(), table_info.get(), table_info->table_.get());
  auto txn_ref = BeginTxn(*bustub, "txn_ref");
  auto txn2 = BeginTxn(*bustub, "txn2");

//   std::cout << "txn2:" << txn2->GetReadTs()  << std::endl;
...
//   std::cout << "txn4:" << txn4->GetReadTs()  << std::endl;
...
TEST(TxnExecutorTest,  UpdateTest1) {  // NOLINT
  ...
  auto txn_ref = BeginTxn(*bustub, "txn_ref");
  auto txn1 = BeginTxn(*bustub, "txn1");
  std::cout << "txn1 readts: " << txn1->GetReadTs() << std::endl;
  std::cout << "txn_ref readts: " << txn_ref->GetReadTs() << std::endl;
  WithTxn(txn1, ExecuteTxn(*bustub, _var, _txn, "INSERT INTO table1 VALUES (1, 1, 1)"));
//   WithTxn(txn1, QueryShowResult(*bustub, _var, _txn, query, IntResult{{1, 1, 1}}));  //
...
TEST(TxnExecutorTest,   UpdateTest2) {  // NOLINT
  ...
  std::cout << "txn0 readts: " << txn0->GetReadTs() << std::endl;
  std::cout << "txn0 committs: " << txn0->GetCommitTs() << std::endl;
  std::cout << "txn1 readts: " << txn1->GetReadTs() << std::endl;
  std::cout << "txn_ref readts: " << txn_ref->GetReadTs() << std::endl;
...
TEST(TxnExecutorTest, GarbageCollection) {  // NOLINT
  ...
  std::cout << "txn_watermark_at_0: " << txn_watermark_at_0->GetReadTs() << std::endl;
  ...
  std::cout << "txn_a: " << txn_a->GetReadTs() << " " << txn_a->GetCommitTs() << std::endl;
  ...
  std::cout << "txn_b: " << txn_b->GetReadTs() << " " << txn_b->GetCommitTs() << std::endl;

  auto txn_watermark_at_1 = BeginTxn(*bustub, "txn_watermark_at_1");
  auto txn_watermark_at_1_id = txn_watermark_at_1->GetTransactionId();

  std::cout << "txn_watermark_at_1: " << txn_watermark_at_1->GetReadTs() << std::endl;
  ...
  std::cout << "txn_2: " << txn2->GetReadTs() << " " << txn2->GetCommitTs() << std::endl;

  BumpCommitTs(*bustub, 2);
  auto txn_watermark_at_2 = BeginTxn(*bustub, "txn_watermark_at_2");
  auto txn_watermark_at_2_id = txn_watermark_at_2->GetTransactionId();

  std::cout << "txn_watermark_at_2: " << txn_watermark_at_2->GetReadTs() << std::endl;
  ...
  std::cout << "txn_3: " << txn3->GetReadTs() << " " << txn3->GetCommitTs() << std::endl;
  ...
  std::cout << "txn_watermark_at_3: " << txn_watermark_at_3->GetReadTs() << std::endl;
