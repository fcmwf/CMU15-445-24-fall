...
class BufferPoolManager {
 public:
  ...
  auto GetNewFrame() -> bool;
  auto CheckPageFrameHeader(page_id_t page_id) -> std::optional<frame_id_t>;
  // for test
  void PrintAllPage2FrameId() {
    for (auto &ele : page_table_) {
      std::cout << "page" << ele.first << "frame" << ele.second << std::endl;
    }
  }
