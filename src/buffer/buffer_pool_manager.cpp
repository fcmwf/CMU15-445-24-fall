...
auto BufferPoolManager::NewPage() -> page_id_t {
  page_id_t nextID = next_page_id_.load();
  // std::cout << nextID << std::endl;
  disk_scheduler_->CreatePage(nextID);
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  if (frames_[frameID].get()->pin_count_ != 0) {
    return false;
  }

  replacer_.get()->SetEvictable(frameID, true);
  replacer_.get()->Remove(frameID);
  free_frames_.push_back(frameID);
  frames_[frameID].get()->Reset();
...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  auto frameId_option = CheckPageFrameHeader(page_id);
  if (!frameId_option.has_value()) {
    return std::nullopt;
  }
  auto frameHeader = frames_[frameId_option.value()];
...
auto BufferPoolManager::CheckedReadPage(page_id_t page_id, AccessType access_type) -> std::optional<ReadPageGuard> {
  ...
  return ReadPageGuard(page_id, frames_[frameId_option.value()], replacer_, bpm_latch_, disk_scheduler_);
}
...
auto BufferPoolManager::FlushPageUnsafe(page_id_t page_id) -> bool {
  ...
  std::shared_ptr<FrameHeader> frameHeader = frames_[page_table_[page_id]];
  if (frameHeader->is_dirty_) {
    auto write_promise = disk_scheduler_->CreatePromise();
    auto write_future = write_promise.get_future();
    if (!frameHeader->io_done_) {
      std::unique_lock<std::mutex> lk(frameHeader->m_);
      frameHeader->cv_.wait(lk, [&]() { return frameHeader->io_done_; });
    }
    disk_scheduler_->Schedule({true, frameHeader->GetDataMut(), frameHeader->page_id_, std::move(write_promise)});
    write_future.get();
    frameHeader->io_done_ = true;
    frameHeader->cv_.notify_all();
  }
  return true;
}
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  if (frameHeader->is_dirty_) {
    ...
    frameHeader->rwlatch_.lock();
    disk_scheduler_->Schedule({true, frameHeader->GetDataMut(), frameHeader->page_id_, std::move(write_promise)});
    write_future.get();
    frameHeader->io_done_ = true;
    frameHeader->rwlatch_.unlock();
    frameHeader->cv_.notify_all();
  }
  return true;
}
...
auto BufferPoolManager::CheckPageFrameHeader(page_id_t page_id) -> std::optional<frame_id_t> {
  std::scoped_lock latch(*bpm_latch_);
  std::shared_ptr<FrameHeader> frameHeader;
  frame_id_t newFrameId;
  std::future<bool> write_future;
  // std::cout << "page_id:  " << page_id << std::endl;
  if (page_table_.find(page_id) != page_table_.end()) {  // page already in memory
    // std::cout << "already in mem" << std::endl;
    newFrameId = page_table_[page_id];
    frameHeader = frames_[newFrameId];
    frameHeader->pin_count_++;
  } else {
    // std::cout << " page in disk" << std::endl;
    // page in disk
    if (free_frames_.empty() && !GetNewFrame()) {
      return std::nullopt;
    }  // frames not enough and can't get new frame
    // pop a new frame from free frames
    newFrameId = free_frames_.back();
    frameHeader = frames_[newFrameId];
    free_frames_.pop_back();

    // page_table change
    page_table_.erase(frameHeader->page_id_);
    page_table_.emplace(page_id, newFrameId);
    // replacer change
    replacer_->RecordAccess(newFrameId);
    replacer_->SetEvictable(newFrameId, false);
    // frame set
    frameHeader->Reset();
    frameHeader->page_id_ = page_id;
    frameHeader->pin_count_.store(1);
    frameHeader->io_done_ = false;

    // process io
    frameHeader->rwlatch_.lock_shared();
    auto write_promise = disk_scheduler_->CreatePromise();
    write_future = write_promise.get_future();
    disk_scheduler_->Schedule(DiskRequest{false, frameHeader->GetDataMut(), page_id, std::move(write_promise)});
    write_future.get();
    frameHeader->io_done_ = true;
    frameHeader->rwlatch_.unlock_shared();
    frameHeader->cv_.notify_all();
  }
  return newFrameId;
}

auto BufferPoolManager::GetNewFrame() -> bool {
  auto newFrame = replacer_->Evict();
  if (!newFrame.has_value()) {
    return false;
  }                                    // no rome for new page frame
  auto newFrameId = newFrame.value();  // get new frame
  auto frameHeader = frames_[newFrameId];
  if (frameHeader->is_dirty_) {  // page should write back to disk
    // std::cout
    // << "pageid:" << frameHeader->page_id_ << std::endl
    // << "frameid" << frameHeader->frame_id_<< std::endl
    // << "current data:" << frameHeader->GetData() << std::endl;
  ...
  frameHeader->io_done_ = true;
  free_frames_.push_back(newFrameId);
  return true;
}
